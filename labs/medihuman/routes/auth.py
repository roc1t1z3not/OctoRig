...
def init(app):
    ...
    def register():
        ...
        if request.method == 'POST':
            ...
            if not (username and password and email):
                ...
                try:
                    db.execute(
                        "INSERT INTO users (username, password, email, full_name, role) VALUES (?, ?, ?, ?, 'patient')",
    ...
    # ── Password reset — weak token: md5(username + epoch) ───────────────────
    ...
    def reset_confirm(token):
        ...
        if not row:
            return render_template('reset_confirm.html', error='Invalid or expired reset link.', token=token)
        error = None
        if request.method == 'POST':
